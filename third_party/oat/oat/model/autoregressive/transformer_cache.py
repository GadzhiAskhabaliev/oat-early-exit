...
class AutoregressiveModel(ModuleAttrMixin):
    ...
    def forward(self, 
        ...
        output: (B, T_tok, vocab_size), or (logits, hidden) if return_hidden
            hidden is post-ln_f, same shape as token sequence (B, T_tok, n_emb).
        """
    ...
    def generate(self, 
        ...
        early_exit_gate: optional module mapping hidden (B, n_emb) -> logit (B, 1);
            if sigmoid(logit) >= early_exit_threshold for all batch rows (after
            early_exit_min_new_tokens new tokens), stop generation early.
        early_exit_max_prob: if set (e.g. 0.95), also stop when max softmax prob of
            the *next-token* distribution exceeds this (no learned gate required).
        output: (B, T_pre + n_generated) with n_generated <= max_new_tokens
        """                
        ...
        for i in range(max_new_tokens):
            ...
            n_new = out_tokens.shape[1] - T_pre
            if n_new < early_exit_min_new_tokens:
                continue

            if early_exit_max_prob is not None and temperature > 0:
                lg = logits.squeeze(1) / temperature
                if top_k is not None:
                ...
                if bool((conf >= early_exit_max_prob).all()):
                    break

            if early_exit_gate is not None:
                h = x.squeeze(1)
                p = torch.sigmoid(early_exit_gate(h).squeeze(-1))
                if bool((p >= early_exit_threshold).all()):
                    break
