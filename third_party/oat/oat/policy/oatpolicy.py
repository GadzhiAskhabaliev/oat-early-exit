...
class OATPolicy(BasePolicy):
    def __init__(
        ...
        topk: int = 10,
        # optional early exit during autoregressive token generation (see oat_ext / docs)
        early_exit_gate: Optional[nn.Module] = None,
        early_exit_loss_weight: float = 0.0,
        early_exit_threshold: float = 0.9,
        early_exit_min_new_tokens: int = 2,
        early_exit_max_prob: Optional[float] = None,
        early_exit_gate_checkpoint: Optional[str] = None,
        # if True, rollout / eval uses early exit without changing call sites
        use_early_exit_inference: bool = False,
    ):
        super().__init__()
        ...
        self.early_exit_min_new_tokens = early_exit_min_new_tokens
        self.early_exit_max_prob = early_exit_max_prob
        self.early_exit_gate_checkpoint = early_exit_gate_checkpoint
        self.use_early_exit_inference = use_early_exit_inference
        if self.early_exit_gate_checkpoint is not None:
            if self.early_exit_gate is None:
                raise ValueError(
                    "early_exit_gate_checkpoint is set, but early_exit_gate module is None. "
                    "Instantiate a gate via Hydra first."
                )
            payload = torch.load(self.early_exit_gate_checkpoint, map_location="cpu")
            state = payload.get("gate_state_dict", payload) if isinstance(payload, dict) else payload
            self.early_exit_gate.load_state_dict(state)
    ...
    def get_optimizer(
        ...
        gate_decay_params = []
        gate_nodecay_params = []
        if self.early_exit_gate is not None:
            for param in self.early_exit_gate.parameters():
                if not param.requires_grad:
                    continue
                if param.dim() >= 2:
                    gate_decay_params.append(param)
                else:
                    gate_nodecay_params.append(param)
        ...
        optim_groups = [
            ...
            {'params': gate_decay_params, 'lr': policy_lr, 'weight_decay': weight_decay},
            {'params': gate_nodecay_params, 'lr': policy_lr, 'weight_decay': 0.0},
    ...
    def predict_action(self, 
        ...
        do_early = use_early_exit or self.use_early_exit_inference
        if do_early and (
            self.early_exit_gate is not None or self.early_exit_max_prob is not None
        ):
            gen_kw.update(
                early_exit_gate=self.early_exit_gate,
                early_exit_threshold=self.early_exit_threshold,
                early_exit_min_new_tokens=self.early_exit_min_new_tokens,
                early_exit_max_prob=self.early_exit_max_prob,
            )
    ...
    def forward(self, batch) -> torch.Tensor:
        ...
        # forward model
        if self.early_exit_gate is not None and self.early_exit_loss_weight > 0:
            logits, hidden = self.model(
                action_tokens[:, :-1], cond=features, return_hidden=True
            )
        else:
        ...
        if self.early_exit_gate is not None and self.early_exit_loss_weight > 0:
            gate_logits = self.early_exit_gate(hidden).squeeze(-1)
            targets = torch.zeros_like(gate_logits)
            targets[:, -1] = 1.0
            loss = loss + self.early_exit_loss_weight * F.binary_cross_entropy_with_logits(
                gate_logits, targets
            )
        return loss
